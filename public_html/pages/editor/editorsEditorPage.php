                ...
                <main class="main row justify-content-center p-3">
                    <div class="accordion col-md-9">
                        <?php
                            try {
                                db_connect();
                                // get an array with all the editors
                                $query1 = "SELECT id_user, names FROM users WHERE rol=?";
                                $editors = get_array($query1, 'editor');
                                ...
                                foreach ($editors as $key => $editor) {
                                    ...
                                    $query2 = "SELECT title, body, id_thread FROM threads WHERE id_user=?";
                                    echo thread_group_by_editor($query2, $key);
